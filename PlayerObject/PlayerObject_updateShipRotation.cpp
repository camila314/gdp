float Slerp2D(float startAngle, float endAngle, float t) {
    ...
    float dot = (sinStart * sinEnd) + (cosStart * cosEnd);
    ...
    if ((1.0f - dot) > 0.0001f) {
        ...
        weightStart = sinf(theta * (1.0f - t)) / sinTheta;
    ...
    float interpSin = (weightEnd * sinEnd) + (weightStart * sinStart);
    float interpCos = (weightEnd * cosEnd) + (weightStart * cosStart);

    return atan2(interpSin, interpCos) * 2.0f;
}
